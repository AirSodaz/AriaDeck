...
impl AppShell {
    ...
    fn submit_transfers(&mut self, cx: &mut Context<Self>) {
        if self.page != AppPage::Settings || self.pending_settings_save.is_some() {
            return;
        }
    ...
    fn read_speed_limit_draft(&self, cx: &Context<Self>) -> SpeedLimitSettingsView {
    ...
    fn read_transfer_policy_draft(&self, cx: &Context<Self>) -> TransferPolicySettingsView {
    ...
    fn sync_speed_limit_fields_from_settings(&mut self, cx: &mut Context<Self>) {
        let speed_limits = self.settings.speed_limits.clone();
        self.settings_download_limit_input.update(cx, |input, cx| {
            input.set_text(speed_limits.download_limit.clone(), cx);
        });
    ...
    fn sync_transfer_policy_fields_from_settings(&mut self, cx: &mut Context<Self>) {
                                            ...
                                            .child(
                                                div()
                                                    ...
                                                    .child(match active_category {
                                                        SettingsCategory::General => self.render_settings_general(cx).into_any_element(),
                                                        SettingsCategory::Profiles => self.render_settings_profiles(cx).into_any_element(),
                                                        SettingsCategory::Engine => self.render_settings_engine(cx).into_any_element(),
                                                        SettingsCategory::Network => self.render_settings_network(cx).into_any_element(),
                                                        SettingsCategory::Transfers => self.render_settings_transfers(cx).into_any_element(),
                                                        SettingsCategory::Notifications => self.render_settings_notifications(cx).into_any_element(),
                                                        SettingsCategory::System => self.render_settings_system(cx).into_any_element(),
                                                    }),
                                            ),
                                    )
                                    .child(render_vertical_scrollbar(&self.settings_scroll, colors)),
                            )
    ...
    fn render_settings_nav(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
    ...
    fn render_settings_footer(&mut self, cx: &mut Context<Self>) -> AnyElement {
        let colors = self.theme.colors;
        let pending = self.pending_settings_save.is_some();
        let active_category = self.settings_page.active_category;
        let error = self.settings_page.error.clone();

        if matches!(active_category, SettingsCategory::Profiles | SettingsCategory::Engine) {
            return div().into_any_element();
        }

        let (dirty, saving) = match active_category {
            SettingsCategory::General => {
                let dirty = self.settings_directory_input.read(cx).text().trim()
            ...
            SettingsCategory::Network => {
                let password_changed =
                    !self.settings_proxy_password_input.read(cx).text().is_empty();
        ...
        div()
            ...
            .child(
                Button::new("footer-save", if saving { "Saving..." } else { "Save" })
                    .aria_label(if saving { "Saving settings" } else { "Save settings" })
                    .style(ButtonStyle::Primary)
    ...
    fn render_settings_general(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
        let pending = self.pending_settings_save.is_some();
        ...
        div()
            ...
            .gap_4()
            .child(
                settings_card("Appearance", colors)
                    .child(settings_row("Theme", None, scheme_control, colors)),
            )
            .child(
                settings_card("Downloads", colors).child(settings_row(
                    "Default directory",
    ...
    fn render_settings_profiles(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
    ...
    fn render_settings_engine(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
    ...
    fn render_settings_network(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
        let pending = self.pending_settings_save.is_some();
        let password_changed =
            !self.settings_proxy_password_input.read(cx).text().is_empty();
    ...
    fn render_settings_transfers(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
        let pending = self.pending_settings_save.is_some();
    ...
    fn render_settings_notifications(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
        let pending = self.pending_settings_save.is_some();
    ...
    fn render_settings_system(&mut self, cx: &mut Context<Self>) -> Div {
        let colors = self.theme.colors;
        let pending = self.pending_settings_save.is_some();
...
fn settings_card(title: &'static str, colors: crate::ThemeColors) -> Div {
    div()
        .flex()
        .flex_col()
        .gap_3()
...
mod tests {
    ...
    fn transfers_footer_saves_speed_limit_and_policy_together(cx: &mut TestAppContext) {
        ...
        view.update(cx, |shell, cx| {
            ...
            shell.settings_max_concurrent_input.update(cx, |input, cx| {
                input.set_text("8", cx);
            });
        });
        view.update(cx, |shell, cx| {
        ...
        view.read_with(cx, |shell, _cx| {
            ...
            assert_eq!(pending.settings.transfer_policy.max_concurrent_downloads, "8");
        });
        assert_eq!(
            events
                .lock()
    ...
    #[gpui::test]
    fn transfers_footer_rejects_invalid_speed_even_when_policy_is_dirty(
        cx: &mut TestAppContext,
    ) {
        let (view, cx) = cx.add_window_view(|window, cx| AppShell::new(Theme::dark(), window, cx));
        view.update_in(cx, |shell, window, cx| {
