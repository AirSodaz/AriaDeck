...
**Status:** Implemented. B3a contract, B3b host + IPC, B3c reference extension, and the cross-cutting settings/installer/i18n work are all in tree. Outstanding: a published extension ID, and the manual end-to-end check against a real gated download (§8).
...
- [x] Icons at 16/32/48/128 px, rendered from the app's own `assets/icon.svg` by `scripts/render-extension-icons.ps1` so the extension and the app cannot drift apart, and pinned by a test that reads the manifest and checks each declared PNG's actual IHDR dimensions — a wrong path or size is otherwise accepted silently and shows as the default puzzle piece. A simplified small-size variant was tried and dropped: without the deck bars the letterform reads as a bare chevron, which looks like a different product beside the app icon.
- [x] The manifest's security shape is asserted rather than reviewed: no install-time host access, no `externally_connectable`, no content scripts, no web-accessible resources, and neither `downloads` nor `tabs` nor `webRequest` requested anywhere (§7, §9).
...
The extension ID is unpublished, so the installer's opt-in task stays hidden until a build supplies `ARIADECK_EXTENSION_ID`; that step is listed in that README.
...
Both installer branches were compiled locally with Inno Setup 6 — with and without `/DBridgeExtensionId` — confirming the task and the `[Run]` register entry appear only when an ID was supplied. The register/unregister round trip was also run against the real `HKCU\...\NativeMessagingHosts` keys under both browsers: it writes the pinned `allowed_origins`, removes both keys and the manifest again, is idempotent, and leaves the unrelated hosts sharing those keys untouched. (The unit test covers the same code against throwaway keys, so the suite never touches what a browser reads.)
